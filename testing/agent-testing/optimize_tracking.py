...
class MiracidiaTracker:
    ...
    def load_video(self) -> None:
        """Load video frames using cv2.VideoCapture."""
        print(f"Loading video: {self.video_path}")
        worm_vid = cv2.VideoCapture(str(self.video_path))
        num_frames = int(worm_vid.get(cv2.CAP_PROP_FRAME_COUNT))
        
        # Limit frames if specified
        if self.max_frames is not None:
            num_frames = min(num_frames, self.max_frames)
            print(f"Loading {num_frames} frames (limited from {int(worm_vid.get(cv2.CAP_PROP_FRAME_COUNT))})")
        else:
        ...
        for i in range(num_frames):
            if i % 50 == 0:
                print(f"  Loading frame {i}/{num_frames}")
            ret, frame = worm_vid.read()
            if not ret:
                break
        ...
        worm_vid.release()
        self.frames = worm_arr
        print(f"Loaded {len(self.frames)} frames, shape: {self.frames.shape}")
    ...
    def subtract_background(self, frame: np.ndarray, background: np.ndarray) -> np.ndarray:
        ...
        """
        # Use absolute difference like in invision-tools
        subtracted = np.absolute((frame - background).astype(np.int16))
    ...
    def generate_background(self, start_frame: int = 0, chunk_size: int = 25) -> np.ndarray:
        ...
        background = np.amax(self.frames[start_frame:end_frame, :, :], axis=0)
        return background
