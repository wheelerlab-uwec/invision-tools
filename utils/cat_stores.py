    ...
    with tqdm(total=length / skip) as pbar:
        ...
        for i in range(0, length, skip):
            ...
            if rescale:
                ...
                merged_array = np.uint16(np.hstack((rescaled_left, right_frame)))
            else:
                merged_array = np.uint16(np.hstack((left_frame, right_frame)))

            if resize and annotate:
                res = cv2.resize(
                    merged_array, dsize=(800, 267), interpolation=cv2.INTER_LINEAR
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "left", type=str, help="Path to the imgstore (directory) from the left camera."
    )
    parser.add_argument(
        "right",
        type=str,
    ...
    parser.add_argument(
        "output", type=str, help="Name of the directory to write the new imgstore."
    )
    parser.add_argument(
        "-l",
        "--length",
